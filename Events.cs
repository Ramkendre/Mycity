...
public class Events : System.Web.Services.WebService
{
    ...
    public string UploadImage(int ID, string imgString, string EventType, string mobileno)
    {
        string Data = "0";
        try
        {
            ...
            if (imgString != "" && imgString != null)
            {
                ...
                string sql = "insert into [come2mycity].[tbl_EventImage](EID,ImgString,EventType,MobileNo,UserId,Status)values('" + ID + "','" + imgString + "','" + EventType + "','" + mobileno + "','" + s + "','0')";
    ...
    public string DownloadImage(string EventType, string mobileno)
    {
        ...
        string s = "Select EID,EventType,ImgString FROM [Come2myCityDB].[come2mycity].[tbl_EventImage] where UserId='" + str1 + "' and EventType='"+EventType+"' and Status='0'";
    ...
    public string DownloadChildImage(string keyword,string RefMobileNo)
    {
        ...
        try
        {
            string str =
                     ...
                     " ) " +
                     " ) ";
            str += " select [firstName],[lastName],m,[usertype],[RefMobileNo],[address],[eMailId],[EID],[EventType],[ImgString],UserId from Event where Status='0'";
            ds = cc.ExecuteDataset(str);
            ...
            if(ds!=null && ds.Tables[0].Rows.Count!=0)
            {
                ...
                for (int row = 0; row < ds.Tables[0].Rows.Count;row++ )
                {
                    if(row==0)
                    {
                        userid=ds.Tables[0].Rows[row][10].ToString();
                    ...
                    {
                        if (!ds.Tables[0].Rows[row][10].ToString().Equals(userid))
                        {
                            userid = ds.Tables[0].Rows[row][10].ToString();
                            header = "$" + ds.Tables[0].Rows[row][0].ToString() + '*' + ds.Tables[0].Rows[row][1].ToString() + '*' +
                                      ds.Tables[0].Rows[row][2].ToString() + '*' + ds.Tables[0].Rows[row][3].ToString() + '*' +
                        ...
                        string str4 = "Update [Come2myCityDB].[come2mycity].[tbl_EventImage] set Status='1' where UserId='" + ds.Tables[0].Rows[row][10].ToString() + "' ";
        ...
        if (!string.IsNullOrEmpty(RefMobileNo) && !string.IsNullOrEmpty(CurrentDate))
        {
            ...
            try
            {
                ...
                string str =
                    ...
                    " ) " +
                    " ) ";
                str += " select [firstName],[lastName],m,[usertype],[RefMobileNo],[address],[eMailId],NID,NewsHead,NewsDetails,NPaper,Role,Date,Time,TypeOfNews,Location,Feedback,CurrentDate,UserId from Event where Status2='0' and (CurrentDate=convert(date,'" + CurrentDate + "') or CurrentDate>convert(date,'" + CurrentDate + "'))order by UserId";
                ds = cc.ExecuteDataset(str);
        ...
        if (!string.IsNullOrEmpty(RefMobileNo) && !string.IsNullOrEmpty(CurrentDate))
        {
            try
            {
                ...
                string str =
                    ...
                    " ) " +
                    " ) ";
                str += " select [firstName],[lastName],m,[usertype],[RefMobileNo],[address],[eMailId],DID,NameOfAccused,Date,Time,Location,SDescp,Relative,Relation,PVisit,MDescp,CurrentDate,UserId from Event where Status2='0' and (CurrentDate=convert(date,'" + CurrentDate + "') or CurrentDate>convert(date,'" + CurrentDate + "'))order by UserId";
                ds = cc.ExecuteDataset(str);
